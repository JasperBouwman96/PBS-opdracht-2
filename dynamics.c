...
*/
{
    ...
    for (i=0; i<p_parameters->N; i++)
    {
        ...
        p_nbrlist->dr[i].sq = (p_nbrlist->dr[i].x)*(p_nbrlist->dr[i].x) + (p_nbrlist->dr[i].y)*(p_nbrlist->dr[i].y)
                              + (p_nbrlist->dr[i].z)*(p_nbrlist->dr[i].z);
    }
}
...
void berendsen_thermostat(struct Parameters *p_parameters, struct Vectors *p_vectors, double Ekin)
/* Change velocities according by Berendsen thermostatting.
   The relaxation time tau is stored in p_parameters->tau. */
{
    int i;
    double  lambda,Ekin0;

    Ekin0 = 1.5*((double)(p_parameters->N-1))*p_parameters->kT;  //expected kinetic energy for desired temperature
    lambda = sqrt(1.0+p_parameters->dt/p_parameters->tau*(Ekin0/Ekin-1.0));
    for (i=0; i<p_parameters->N; i++)
    {
